...
def run(fold,
    ...
    for file in config.TRAINING_FILES:
        ...
        if features:
            features_to_keep = config.FEATURES
            features_to_keep.append("kfold")
        ...
        bal_acc = metrics.balanced_accuracy_score(y_valid, preds)
        print(f"Fold={fold}, F1Score={f1_score}")
        print(f"Fold={fold}, BalancedAccuracyScore={bal_acc}")
